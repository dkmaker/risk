...
import { getCurrentLanguage, setLanguage, t } from "./translations.js";
...
function selectAttackerArmy(count) {
  ...
  document.querySelectorAll("#attacker-army-buttons .army-btn").forEach((btn) => {
    ...
    if (Number.parseInt(btn.textContent) === count) {
  ...
  if (page === 0) {
    ...
    buttons.forEach((btn, index) => {
      ...
      if (index === 0) {
        ...
        btn.setAttribute(
          "onclick",
          page < 5 ? "changeAttackerPage(1)" : `selectAttackerArmy(${startNum + 8})`
        );
  ...
  if (page === 0) {
    ...
    buttons.forEach((btn, index) => {
      ...
      if (index === 0) {
        ...
        btn.setAttribute(
          "onclick",
          page < 5 ? "changeDefenderPage(1)" : `selectDefenderArmy(${startNum + 8})`
        );
...
function clearBattleScreen() {
  ...
  document
    ...
    .classList.remove("attacker-wins", "defender-wins", "both-lose");
...
function updateUILanguage() {
  ...
  if (
    resultMessage.textContent === "Click Roll to start battle!" ||
